...
def format_image(image, vid=False, file=None):
    ...
    if vid:
        ...
        for face_coor in faces:
            ...
            try:
                new_image = cv2.resize(new_image, (48, 48), interpolation=cv2.INTER_AREA)
        ...
        if result is not None:
            ...
            for c in range(0, 3):
                frame[200:320, 10:130, c] = emoji_face[:, :, c] * (emoji_face[:, :, 3] / 255.0) + \
                                            frame[200:320, 10:130, c] * (1.0 - emoji_face[:, :, 3] / 255.0)
